    ...
    <div v-if="unavailableType && ! isLoading" class="flex items-center justify-center">
        <ns-notice color="warning">
            ...
            <template #description>
                {{ __( 'This transaction was created with a type that is no longer available. This type is no longer available because RazanPOS is unable to perform background requests.' ) }}
            </template>
            <template #controls>
                ...
                if ( this.transaction.type === undefined ) {
                    ...
                    if ( expenseConfiguration.length == 0 ) {
                        ...
                        return Popup.show( nsAlertPopup, {
                            ...
                            message: __( 'You cannot edit this transaction if RazanPOS cannot perform background requests.' )
                        });
                    }
