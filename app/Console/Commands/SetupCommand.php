...
class SetupCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Install RazanPOS from the command line.';
    ...
    public function handle()
    {
        ...
        if (in_array(strtolower($answer), ['y', 'yes']) || $this->requireConfirmation === false) {
            ...
            return $this->info('Thank you, RazanPOS has been successfully installed.');
        } else {
    ...
    private function setupLanguage()
    {
        while (empty($this->language)) {
            $langIndex = $this->choice(__('In which language would you like to install RazanPOS ?'), array_values(config('nexopos.languages')));
